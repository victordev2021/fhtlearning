<article>
    <h1 class="uppercase text-2xl font-bold">audiencias del curso</h1>
    <hr class="mt-2 mb-6">
            ...
            <div class="card-body bg-gray-100">
                @if ($audience->id == $item->id)
                    <form wire:submit.prevent='update'>
                        <input wire:model='audience.name' class="form-input w-full" type="text">
                        @error('audience.name')
    ...
    <article class="card">
        <div class="card-body bg-gray-100">
            <form wire:submit.prevent='store'>
                <input wire:model='name' class="form-input" type="text" placeholder="Agregar nombre del audiencia.">
                @error('name')
                    <span class="text-xs text-red-500">{{ $message }}</span>
                @enderror
